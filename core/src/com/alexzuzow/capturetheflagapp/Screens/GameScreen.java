...
public class GameScreen extends ApplicationAdapter implements Screen{
    ...
    private Texture bluePlayerSprite;
    private Texture redPlayerSprite;
    ...
    public GameScreen(CaptureTheFlagApp game) {
        ...
        bluePlayerSprite = new Texture(Gdx.files.internal("BluePlayerSprite.png"));
        redPlayerSprite= new Texture(Gdx.files.internal("RedPlayerSprite.png"));
    ...
    public void render(float delta) {
        ...
        for (HashMap.Entry<Integer, User> entry : CaptureTheFlagApp.playerList.entrySet()) {
            if (entry.getValue().isAlive()) {
                ...
                if (entry.getValue().hasFlag() && entry.getValue().isBlueTeam()) {
                    redFlagSprite.draw(game.batch);

                } else if (entry.getValue().hasFlag() && entry.getValue().isRedTeam()) {
    ...
    public void update(float dt) {
        ...
        for (BufferAddNewUser player : CaptureTheFlagApp.newPlayers) {
            ...
            if (!CaptureTheFlagApp.playerList.containsKey(player.clientID)) {
                ...
                User newPlayer = new User(this, player.xCoord, player.yCoord, playerSprite,player.isRedTeam,player.isBlueTeam);
...
//            System.out.println("(X,Y): "+player.getValue().getxCoord()+" "+player.getValue().getyCoord());
            ...
            //for some reason puts new players *100 distance
            ...
            if (player.getValue().hasMoved()) {
                ...
                updatePlayer.isRedTeam = player.getValue().isRedTeam();
                updatePlayer.isBlueTeam = player.getValue().isBlueTeam();
...
//        System.out.println("resize Y: " + height);
    ...
    public void createPlayer() {
        ...
        if (!CaptureTheFlagApp.playerList.containsKey(CaptureTheFlagApp.clientID)) {
            ...
            if (CaptureTheFlagApp.redTeamSize <= CaptureTheFlagApp.blueTeamSize) {
                ...
                for (SpawnPoint redSpawn : creator.getRedPlayerSpawns()) {
                    if (redSpawn.isSafeSpawnPoint()) {
                        ...
                        Texture playerSprite = redPlayerSprite;

                        CaptureTheFlagApp.playerList.put(CaptureTheFlagApp.clientID, new User(this,
                                redSpawn.getX() * CaptureTheFlagApp.PPM, redSpawn.getY() * CaptureTheFlagApp.PPM,
                                playerSprite,true,false));
                ...
                for (SpawnPoint blueSpawn : creator.getBluePlayerSpawns())
                    ...
                    if (blueSpawn.isSafeSpawnPoint()) {
                        ...
                        Texture playerSprite = bluePlayerSprite;
                        CaptureTheFlagApp.playerList.put(CaptureTheFlagApp.clientID, new User(this,
                                blueSpawn.getX() * CaptureTheFlagApp.PPM, blueSpawn.getY() * CaptureTheFlagApp.PPM,
                                playerSprite,false,true));
                        CaptureTheFlagApp.playerList.get(CaptureTheFlagApp.clientID).setClinetID(CaptureTheFlagApp.clientID);
            ...
            newPlayerServer.isRedTeam = CaptureTheFlagApp.playerList.get(CaptureTheFlagApp.clientID).isRedTeam();
            newPlayerServer.isBlueTeam = CaptureTheFlagApp.playerList.get(CaptureTheFlagApp.clientID).isBlueTeam();
